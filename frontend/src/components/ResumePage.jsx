    ...
    <div className="space-y-8">
      ...
      <div className="bg-gradient-to-r from-brand-950 via-brand-900 to-indigo-950 text-white rounded-2xl p-6 md:p-8 shadow-xl border border-brand-800/50 relative overflow-hidden">
        ...
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6 relative z-10">
          ...
          {/* Upload Controls */}
          <div className="bg-white/5 backdrop-blur-md/10 backdrop-blur-md p-4 rounded-xl border border-white/10 flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
            <select
              value={targetRole}
            ...
            <button
              ...
              className="bg-emerald-500 hover:bg-brand-400 disabled:opacity-50 disabled:cursor-not-allowed text-white font-semibold px-5 py-2.5 rounded-lg text-xs transition flex items-center justify-center gap-2 shadow-lg shadow-brand-500/30 whitespace-nowrap"
            >
        ...
        <div className="flex gap-2">
          <button
            ...
            className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition ${
              activeTab === 'profile'
                ? 'bg-white/5 backdrop-blur-xl border-r border-white/10 text-white shadow-[0_0_15px_rgba(0,0,0,0.2)]'
      ...
      {activeTab === 'profile' && (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="space-y-6">
            {/* Identity Card */}
            <div className="bg-white/5 backdrop-blur-md p-6 rounded-2xl border border-white/10 shadow-[0_0_15px_rgba(0,0,0,0.2)] space-y-4">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h3 className="text-xl font-bold text-white">{profile.name || 'Candidate Name'}</h3>
                  <span className="inline-block mt-1 px-2.5 py-0.5 bg-emerald-500/10 text-emerald-300 font-semibold text-xs rounded-full border border-brand-500/20">
            ...
            <div className="bg-white/5 backdrop-blur-md p-6 rounded-2xl border border-white/10 shadow-[0_0_15px_rgba(0,0,0,0.2)] space-y-4">
              ...
              {categorizedSkills ? (
                  ...
                  <div key={category} className="space-y-2">
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {Array.isArray(skills) && skills.map((skill) => (
                        <span key={skill} className="px-2.5 py-1 bg-white/10 hover:bg-white/20 text-slate-200 text-xs font-medium rounded-md transition">
                          {skill}
                        </span>
                      ))}
                    </div>
                ...
                <div className="flex flex-wrap gap-1.5">
                  {allSkills.map((skill) => (
                    <span key={skill} className="px-2.5 py-1 bg-emerald-500/10 text-emerald-300 border border-brand-500/20 text-xs font-medium rounded-md">
                      {skill}
                    </span>
                  ))}
                </div>
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {/* Education */}
              <div className="bg-white/5 backdrop-blur-md p-6 rounded-2xl border border-white/10 shadow-[0_0_15px_rgba(0,0,0,0.2)] space-y-3">
                <h4 className="font-bold text-white flex items-center gap-2">
                ...
                {educationList.length > 0 ? (
                  educationList.map((edu, idx) => (
                    <div key={idx} className="space-y-1 border-l-2 border-brand-500 pl-3">
                      <p className="font-semibold text-xs text-white">{edu.degree}</p>
                ...
                {projectList.length > 0 ? (
                  projectList.map((proj, idx) => (
                    <div key={idx} className="space-y-1">
                      <p className="font-semibold text-xs text-white">{proj.name}</p>
      ...
      {activeTab === 'insights' && (
        <div className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {/* ATS Score Card */}
            <div className="bg-white/5 backdrop-blur-md p-6 rounded-2xl border border-white/10 shadow-[0_0_15px_rgba(0,0,0,0.2)] text-center flex flex-col justify-center items-center">
              <div className="relative w-28 h-28 flex items-center justify-center my-2">
                ...
                <div className="absolute flex flex-col items-center">
                  <span className="text-2xl font-extrabold text-white">{atsScore}%</span>
                  <span className="text-[10px] text-slate-400 uppercase font-bold">ATS Match</span>
                </div>
              </div>
              <p className="text-xs text-slate-400 mt-2">Resume compatibility for target role <strong className="text-slate-200">{profile.target_role || 'Candidate'}</strong>.</p>
            </div>
      ...
      {activeTab === 'raw' && (
        <div className="bg-slate-950 text-slate-200 p-6 rounded-2xl border border-slate-800 font-mono text-xs overflow-x-auto shadow-2xl relative">
          <div className="flex justify-between items-center pb-4 mb-4 border-b border-slate-800 text-slate-400">
            ...
            <button
              onClick={handleCopyJSON}
              className="text-xs bg-white/10 hover:bg-slate-700 text-slate-200 px-3 py-1 rounded transition"
            >
              {copied ? 'Copied' : 'Copy'}
            </button>
          </div>
